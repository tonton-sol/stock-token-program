...
fn is_daylight_saving_time(datetime: DateTime<Utc>) -> bool {
    ...
    let dst_start = find_nth_weekday_in_month(year, 3, 0, 2);
    let dst_end = find_nth_weekday_in_month(year, 11, 0, 1);
...
fn find_nth_weekday_in_month(year: i32, month: u32, weekday: u32, nth: u32) -> u32 {
    let mut count = 0;
    for day in 1..32 {
        ...
        if date.weekday().num_days_from_sunday() as u32 == weekday {
            count += 1;
            if count == nth {
                return day;
            }
